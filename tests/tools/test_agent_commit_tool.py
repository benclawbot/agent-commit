"""Tests for the agent_commit tool module."""
...
from tools.agent_commit_tool import (
    CommitStore,
    compute_commit_id,
    generate_summary,
    agent_commit_tool,
...
class TestGenerateSummary:
    ...
    def test_from_tool_calls_when_no_content(self):
...
class TestCommitStore:
    def test_commit_and_get(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert commit.agent_id == "agent-1"
    ...
    def test_commit_id_collision_returns_same_id(self):
        """Identical commits produce identical IDs (deduplication)."""
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # ID is same since content is same
            assert c1.id == c2.id
...
class TestAgentCommitTool:
    ...
    def test_reference_requires_commit_id(self):
        result = agent_commit_tool(action="reference", commit_id="notexist")
        d = json.loads(result)
        assert "error" in d or "reasoning" in d  # returns error or empty ref
...
class TestAgentCommitSchema:
    def test_schema_has_all_actions(self):
        param = AGENT_COMMIT_SCHEMA["parameters"]
        action_enum = param["properties"]["action"]["enum"]
        assert "commit" in action_enum
        assert "log" in action_enum
        assert "diff" in action_enum
        assert "branch" in action_enum
        assert "reference" in action_enum
        assert "get" in action_enum
