...
parent chain, and full context. Supports commit, log, diff, branch, merge,
and reference operations.

Built as a tool that auto-registers via tools/registry.py.
"""
...
class Commit:
    ...
    id: str              # sha256 of canonical content
...
def _serialize_for_hash(messages, tool_calls, tool_results, reasoning, metadata) -> bytes:
...
def compute_commit_id(
    ...
    """Compute a sha256 content hash for a reasoning step."""
    data = _serialize_for_hash(messages, tool_calls, tool_results, reasoning, metadata)
...
def _get_db_path() -> Path:
    home = Path(os.environ.get("HERMES_HOME", str(get_hermes_home())))
...
def _init_db(db_path: Optional[Path] = None) -> sqlite3.Connection:
    ...
    """)
    conn.execute("""
        CREATE INDEX IF NOT EXISTS idx_task_id ON commits(task_id)
    """)
    conn.execute("""
        CREATE INDEX IF NOT EXISTS idx_agent_id ON commits(agent_id)
    """)
    conn.commit()
    return conn
...
class CommitStore:
    """Thread-safe commit store with SQLite backend."""
    ...
    def diff(self, commit_a_id: str, commit_b_id: str) -> Dict[str, Any]:
        ...
        # Compare messages (just count + first/last)
        return {
            "a": {"id": a.id, "timestamp": a.timestamp, "n_messages": len(a.messages)},
            "b": {"id": b.id, "timestamp": b.timestamp, "n_messages": len(b.messages)},
            "tool_calls": {"added": added, "removed": removed, "changed": changed},
            "reasoning_a": a.reasoning,
            "reasoning_b": b.reasoning,
        }

    def branch(self, commit_id: str, new_task_id: str, new_agent_id: str) -> Commit:
        """Create a new commit chain starting from an existing commit."""
        parent = self.get_commit(commit_id)
        if not parent:
            return None

        # Create a new commit that inherits parent's content but has new IDs
        new_commit_id = compute_commit_id(
            parent.messages,
            parent.tool_calls,
            parent.tool_results,
            parent.reasoning,
            {**parent.metadata, "branched_from": commit_id},
        )
        ...
        commit = Commit(
            ...
            metadata={**parent.metadata, "branched_from": commit_id},
    ...
    def _row_to_commit(self, row) -> Commit:
        # Support both sqlite3.Row (dict-like) and tuple access
        if hasattr(row, 'keys'):
            # sqlite3.Row with column names
            return Commit(
                id=row["id"],
        ...
        # Tuple fallback — map by index from cursor.description
        ...
        return Commit(
            ...
            tool_results=json.loads(d["tool_results"]),
            reasoning=json.loads(d["reasoning"]) if d["reasoning"] else None,
...
def agent_commit_tool(
    ...
    **kwargs,
    ...
    if action == "commit":
        ...
        result = store.diff(commit_a_id, commit_b_id)
        ...
        return json.dumps({
            ...
            "valid_actions": ["commit", "log", "diff", "branch", "reference", "get"],
        })
...
def check_agent_commit_requirements() -> bool:
...
AGENT_COMMIT_SCHEMA = {
    ...
    "description": (
        ...
        "agent reasoning commits. Use 'commit' to save a reasoning step with full context. "
        "Use 'log' to see the commit chain for a task. Use 'diff' to compare two commits. "
        "Use 'branch' to fork a new reasoning chain from an existing commit. "
        "Use 'reference' to retrieve full context from a commit for injection into a new session.\n\n"
        "Actions:\n"
        ...
        "  branch    — fork a new chain from an existing commit\n"
        "  reference — retrieve full context from a commit for new session\n"
        "  get       — retrieve a single commit by ID\n\n"
        "Metadata field supports: model, provider, token_count, turn_number, etc."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
registry.register(
    name="agent_commit",
    toolset="file",
    schema=AGENT_COMMIT_SCHEMA,
    handler=lambda args, **kw: agent_commit_tool(
        ...
        commit_a_id=args.get("commit_a_id"),
        commit_b_id=args.get("commit_b_id"),
        new_task_id=args.get("new_task_id"),
        new_agent_id=args.get("new_agent_id"),
        limit=args.get("limit", 50),
    ),
    check_fn=check_agent_commit_requirements,
    emoji="🧠",
)